...
def run_all_knn(X, y, X_train, y_train, X_test, y_test):
    ...
    recall = []
    accuracy = []
    for n in range(1,k_neighbors,3):
...
def draw_crossval_graph(res1, res2, k_values, balance):
    ...
    f.savefig("%s%s_knn_%s.png" % (GRAPHS_FOLDER,"experts0",balance),bbox_inches="tight")
    f.savefig("%s%s_knn_%s.pdf" % (GRAPHS_FOLDER,"experts0",balance),bbox_inches="tight")
